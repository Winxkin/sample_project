...
int sx1278_receive_packet(uint8_t *buf, int size)
{
    ...
    * Find packet size.
    */
   if (__implicit) len = sx1278_read_reg(REG_PAYLOAD_LENGTH);
   else len = sx1278_read_reg(REG_RX_NB_BYTES);
...
{
   if(sx1278_read_reg(REG_IRQ_FLAGS) & IRQ_RX_DONE_MASK) return 1;
   return 0;
}
